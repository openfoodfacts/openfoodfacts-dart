...
void main() {
  ...
  group('$OpenFoodSearchAPIClient autocomplete', () {
    ...
    test('all fuzziness levels', () async {
    ...
    test('all taxonomy names', () async {
      ...
      await simpleTest(
        TaxonomyName.foodGroup,
        'fromage per',
        'Fromage persillé',
      );
      await simpleTest(
        TaxonomyName.improvement,
        'Nutrition - Hau',
        'Nutrition - Haut taux de sel pour la catégorie',
      );
      await simpleTest(TaxonomyName.ingredient, 'fromage bla', 'Fromage blanc');
      await simpleTest(TaxonomyName.ingredientAnalysis, 'végé', 'Végétarien');
      await simpleTest(TaxonomyName.ingredientProcessing, 'enri', 'enrichi');
      await simpleTest(TaxonomyName.language, 'fran', 'français');
      await simpleTest(TaxonomyName.mineral, 'zi', 'Zinc');
      await simpleTest(TaxonomyName.misc, 'nutriscore', 'NutriScore - Calculé');
      await simpleTest(
        TaxonomyName.novaGroup,
        'aliments',
        'Aliments transformés',
      );
      await simpleTest(
        TaxonomyName.nucleotide,
        'adénosine mon',
      ...
      await simpleTest(
        TaxonomyName.otherNutritionalSubstance,
        'chol',
        'Choline',
      );
      await simpleTest(TaxonomyName.packagingMaterial, 'verr', 'Verre');
      await simpleTest(
        TaxonomyName.packagingRecycling,
        'compost',
        'Compostable',
      );
