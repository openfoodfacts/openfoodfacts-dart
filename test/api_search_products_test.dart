...
void main() {
  ...
  group(
    '$OpenFoodAPIClient search products',
    () {
      ...
      /// different orders.
      test(
        'search with all sort-by options',
        () async {
      ...
      Future<int?> checkIngredientsAnalysis(
        ...
        for (final Product product in result.products!) {
          if (veganStatus != null) {
            expect(product.ingredientsAnalysisTags!.veganStatus, veganStatus);
          }
      ...
      test(
        'check random vegan+vegetarian+palm oil search',
        () async {
          final Random random = Random();
          final int veganIndex = random.nextInt(VeganStatus.values.length);
          final int vegetarianIndex = random.nextInt(
            VegetarianStatus.values.length,
          );
          final int palmOilFreeIndex = random.nextInt(
            PalmOilFreeStatus.values.length,
          );
      ...
      test(
        'check vegan/vegetarian consistency',
        () async {
          const VegetarianStatus nonVegetarian =
      ...
      test('search products by keywords 2', () async {
        ...
        SearchResult result = await searchProductsInProd(configuration);
      ...
      test('search products with filter on all tags (part 1)', () async {
        ...
        final parameters = <Parameter>[
          ...
          TagFilter.fromType(
            tagFilterType: TagFilterType.LABELS,
            tagName: labels,
          ),
          TagFilter.fromType(
            tagFilterType: TagFilterType.ORIGINS,
            tagName: originsTags,
          ),
          TagFilter.fromType(
          ...
          TagFilter.fromType(
            tagFilterType: TagFilterType.ALLERGENS,
            tagName: allergens,
          ),
          TagFilter.fromType(
            tagFilterType: TagFilterType.TRACES,
            tagName: traces,
          ),
          TagFilter.fromType(
            tagFilterType: TagFilterType.NUTRITION_GRADES,
          ...
          TagFilter.fromType(
            tagFilterType: TagFilterType.CREATOR,
            tagName: creator,
          ),
          TagFilter.fromType(
      ...
      test('search products with filter on all tags (part 3)', () async {
        ...
        final parameters = <Parameter>[
          TagFilter.fromType(
            tagFilterType: TagFilterType.PACKAGING,
            tagName: packaging,
          ),
        ];
      ...
      test('multiple products and pagination', () async {
        ...
        while (true) {
          ...
          final result = await searchProductsInProd(configuration);
  ...
  List<int> getRandomInts({required int count, required final int max}) {
    final Random random = Random();
  ...
  group(
    '$OpenFoodAPIClient search products with/without allergens',
    () {
      ...
      test('check products with 2 random states tags', () async {
        Future<void> checkExpectations(
          ...
          final int count1 = await checkStatesTags(
            StatesTagsParameter(map: {state1: completed1}),
          );
          final int count2 = await checkStatesTags(
            StatesTagsParameter(map: {state2: completed2}),
          );
  ...
  group(
    '$OpenFoodAPIClient get all to-be-completed products',
    () {
      Future<int?> getCount(
        ...
        try {
          result = await searchProductsInProd(configuration);
        } catch (e) {
  ...
  group(
    '$OpenFoodAPIClient get user products',
    () {
  ...
  group(
    '$OpenFoodAPIClient get new packagings field',
    () {
  ...
  group('$OpenFoodAPIClient matched product v2', () {
    ...
    test('matched product', () async {
      ...
      final List<MatchedProductV2> actuals = MatchedProductV2.sort(
        products,
        manager,
      );
